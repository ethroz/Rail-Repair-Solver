...
class FixedChainQueue {
private:
    static constexpr Index NO_INDEX = std::numeric_limits<Index>::max();
public:
    constexpr FixedChainQueue() = default;

    FixedChainQueue(const FixedChainQueue&) = delete;
    FixedChainQueue& operator=(const FixedChainQueue&) = delete;

    FixedChainQueue(FixedChainQueue&&) = delete;
    FixedChainQueue& operator=(FixedChainQueue&&) = delete;
    ...
    struct chain_iterator {
        ...
        constexpr chain_iterator(const FixedChainQueue& owner, Index index) :
            m_owner(owner),
            m_index(index)
        {}
...
private:
    [[nodiscard]] static constexpr Index indexCast(size_t value) {
        if (value >= size_t(NO_INDEX)) {
            throw std::overflow_error("FixedChainQueue index overflow");
        }
        return Index(value);
    }
    ...
    constexpr void remapPrevIndices(
        ...
        for (size_t i = 0; i < container.size(); ++i) {
            ...
            assert(prevIndex < maxIndex); // FixedChainQueue contains invalid prevIndex

            const Index newPrevIndex = remap[prevIndex];
            assert(newPrevIndex != NO_INDEX); // FixedChainQueue pruning removed a required parent
    ...
    friend class PriorityChainQueue;
